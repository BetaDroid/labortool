...
#### Begin of Schems ####
class ComponentParamSchema(ma.ModelSchema):
    class Meta:
        fields = ('ComponentParamId', 'ComponentParamComponent', 'ComponentParamCategoryParamType', \
                  'ComponentParamValue', 'ComponentParamPrefix', 'ComponentParamNote')
...
def getComponentParam(ID):  
    cps = mysql.engine.execute("select * from componentparam where ComponentParamComponent = %d" % ID)

    component_param_schema = ComponentParamSchema(many=True)
    result = component_param_schema.dump(cps)
